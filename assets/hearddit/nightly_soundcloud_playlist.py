...
def get_submissions(subreddit="electronicmusic", limit=100, session=None):
    if not session:
        r = praw.Reddit(
            "hearddit", user_agent="get_top; subreddit={0}".format(subreddit)
        )
    else:
        r = session
...
def check_reposts_and_submit_url(
    creds_file, subreddit, title, playlist_url, username="heardditbot"
):
    ...
    logger.info(
        "url_was_already_submitted: {}".format(
            url_was_already_submitted(submission_hash)
        )
    )
    if not url_was_already_submitted(submission_hash):
        ...
        submission.add_comment(
            """Hearddit builds playlists from links posted on music subreddits. Details: http://ryancompton.net/2014/12/23/hearddit/ """
        )
...
def create_soundcloud_playlist_from_urls(urls, playlist_name):
    ...
    expired_pls = [
        ...
        if dateutil.parser.parse(pl.fields()["created_at"]).replace(tzinfo=None) < dto
    ]
...
def search_spotify_for_a_title(title, sp):
    ...
    if len(query) > 5:  # search for 5+ char strings
        ...
        if len(results["tracks"]["items"]) > 0:
            ...
            if (hit_artist.lower() in query.lower()) and (
                hit_title.lower() in query.lower()
            ):
                logger.info(
                    "hit accepted: query={0} \t title={1} \t artist={2}".format(
                        query, hit_title, hit_artist
                    )
                )
            else:
                logger.info(
                    "hit rejected: query={0} \t title={1} \t artist={2}".format(
                        query, hit_title, hit_artist
                    )
                )
            return results
...
def create_spotify_playlist_from_titles(todays_titles, playlist_name):
    ...
    for x in todays_titles:
        try:
            ...
            pass
    search_results = [
        x for x in search_results if x and (len(x["tracks"]["items"]) > 0)
    ]
    ...
    logger.warning(
        "adding {0} new tracks to {1}".format(len(new_track_uris), new_pl["name"])
    )
    if len(new_track_uris) > 0:
        for sublist in chunker(new_track_uris, 99):
            ...
            time.sleep(7)
            logger.warning(
                "added {0} new tracks to {1}".format(len(sublist), new_pl["name"])
            )
...
def main():
    ...
    urls = [
        (x.url, x.title, datetime.datetime.fromtimestamp(x.created)) for x in hot_links
    ]
    ...
    try:
        ...
        new_soundcloud_list_url = create_soundcloud_playlist_from_urls(
            todays_urls, playlist_name
        )
    ...
    try:
        new_spotify_list_url = create_spotify_playlist_from_titles(
            todays_titles, playlist_name
        )
    ...
    if today == wednesday:
        ...
        if (subreddit in allowed_subreddits) and (new_soundcloud_list_url is not None):
            ...
            logger.info(
                ...
                + subreddit
                + " url: "
                + new_soundcloud_list_url
            )
            check_reposts_and_submit_url(
                creds_file="/home/ubuntu/my_reddit_accounts.properties",
                subreddit=subreddit,
                title=link_title,
                playlist_url=new_soundcloud_list_url,
                username=botname,
            )
            ...
            logger.info(
                ...
                + subreddit
                + " url: "
                + new_spotify_list_url
            )
            check_reposts_and_submit_url(
                creds_file="/home/ubuntu/my_reddit_accounts.properties",
                subreddit=subreddit,
                title=link_title,
                playlist_url=new_spotify_list_url,
                username=botname,
            )
