...
def build_address_map(limit=5):
    ...
    df = pd.read_sql(
        'SELECT mapaddress, "geo.region", post_hood, data_accuracy FROM cragprod WHERE "geo.region" = \'US-CA\' and data_accuracy > 9 LIMIT {}'.format(
            limit
        ),
        engine,
    )
    ...
    for idx, row in df.iterrows():
        try:
            craig_hash = address_hash(
                (row["mapaddress"], row["geo.region"], row["post_hood"])
            )
            if (
                geo_already_done is None
                or craig_hash not in geo_already_done["craig_address_hash"].values
            ):
...
=======
    ...
    logger.warning(
        "starting google geocoder, df.shape: {0} rows: {1} cache_hits: {2}".format(
...
def main():
    ...
    dfrent = pd.read_sql(
        'SELECT post_price, post_date, post_bedroom_count, post_sqft, price_per_sqft, netloc, housing, furnished, crawl_date, mapaddress, "geo.region", post_hood from cragprod WHERE "geo.region" = \'US-CA\' and data_accuracy > 9',
        engine,
    )
    dfrent = dfrent.drop_duplicates()
    dfrent["craig_address_hash"] = dfrent[
        ["mapaddress", "geo.region", "post_hood"]
    ].apply(address_hash, axis=1)
