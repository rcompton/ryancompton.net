...
parser = argparse.ArgumentParser(
    description="Parse the crawled data in S3 to the database."
)
    ...
    for line in smart_open.smart_open(s3_fname):
        ...
        try:
            if "mapaddress" in dic and "geo_region" in dic and "post_hood" in dic:
                dic["clean_address"] = geocode(
                    dic["mapaddress"], dic["geo_region"], dic["post_hood"]
                )
        except:
            logger.exception("geo meh")
        if "price" in dic and (pd.isna(dic["post_price"])):
            dic["post_price"] = float(
                price_dec(dic["price"])
            )  # should not use floats for money but whatever
        dics.append(dic)
...
def parse_dir(date, s3dir="s3://rycpt-crawls/craigslist-housing/"):
    ...
    while partial_list["IsTruncated"]:
        ...
        partial_list = s3.list_objects_v2(
            Bucket=s3_bucket, Prefix=s3_prefix, ContinuationToken=next_token
        )
        obj_list.extend(partial_list["Contents"])
    s3_fnames = [
        f"s3://{s3_bucket}/" + x["Key"] for x in obj_list if date in x["Key"]
    ]  # date filter brittle af
...
def postprocess_df(df):
    ...
    df["data_accuracy"] = df["data_accuracy"].map(
